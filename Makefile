...
# as .theos/obj/Mond2Embedded.dylib by the legacy-named
# scripts/build-mond-2.0-embedded.sh. Keeping it in a separate module avoids
# source rewriting and symbol collisions with Filza's independently pinned
# bad_query implementation, while allowing Mond's iOS 17 deployment target.
...
# patched: the only Mond-specific build step stages the immutable 2.1 commit and
...
before-FilzaApplySandboxExt-all::
	...
	@test -f "scripts/stage-mond-2.0.sh" || (echo "Missing exact Mond 2.1 staging script (legacy filename)" >&2; exit 1)
	@test -f "scripts/build-mond-2.0-embedded.sh" || (echo "Missing exact Mond 2.1 build script (legacy filename)" >&2; exit 1)
