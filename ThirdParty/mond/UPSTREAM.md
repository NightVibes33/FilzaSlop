...
`scripts/stage-mond-2.0.sh` is a legacy-named integration script that now checks out the immutable Mond **2.1** revision under `.theos/mond2/` and verifies that Mond, PartyUI, and ZIPFoundation remain clean upstream checkouts. No generated/namespaced Mond copy is produced and no script edits Mond's Swift/C source.

`scripts/build-mond-2.0-embedded.sh` is likewise legacy-named; it compiles the untouched upstream Mond 2.1 tree into the isolated `Mond2Embedded.dylib`. Isolation prevents symbol/ABI collisions with Filza's independently pinned components and allows upstream Mond's iOS 17 target while the outer Filza/ByeTunes build remains on its existing iOS 16 minimum.
